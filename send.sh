...
#	curl -X PUT "http://192.168.15.103:8080/book?id=$book&name=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)&publishingYear=$(cat /dev/urandom | tr -dc '0-9' | fold -w 4 | head -n 1)&annotation=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)&authors=$author"
	customer=$(curl -X POST "http://192.168.15.103:8080/customer?name=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)&phone=+7$(cat /dev/urandom | tr -dc '0-9' | fold -w 10 | head -n 1)")
	order=$(curl -X POST "http://192.168.15.103:8080/order?name=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)&clientId=$customer")
done
